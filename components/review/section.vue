  ...
  <section class="py-8 md:py-12 text-pb-text">
    <div>
      ...
      <div v-else-if="!isLoading && initialLoadError" class="text-center py-10">
        <p class="text-xl text-pb-text-secondary">
          Không thể tải bình luận. Vui lòng thử lại sau.
        </p>
        <Icon
          icon="mdi:emoticon-sad-outline"
          class="h-12 w-12 mx-auto mt-4 text-pb-text-secondary"
        />
      </div>
      <div
        v-else-if="isLoading && hotReviews.length === 0"
        class="text-center py-10"
      >
        <Icon
          icon="eos-icons:loading"
          class="h-12 w-12 mx-auto text-pb-accent"
        />
        <p class="mt-2 text-pb-text-secondary">Đang tải bình luận...</p>
      </div>

      <div
        v-if="!allServerMoviesLoaded && hotReviews.length > 0"
      ...
      <div
        v-if="
          allServerMoviesLoaded && hotReviews.length > 0 && !initialLoadError
        "
        class="text-center mt-8 md:mt-12 text-pb-text-secondary"
      >
...
const fetchAndProcessReviews = async (isInitialLoad: boolean) => {
  ...
  try {
    while (
      newValidReviewsCollectedInThisBatch.length < reviewsToFindThisBatch &&
      !allServerMoviesLoaded.value
    ) {
      const moviesResponse = await fetch(
        `${API_BASE_URL}/movies?_page=${currentPage.value}&_limit=${serverFetchLimit}&_sort=releaseYear&_embed=comments`
      );
      ...
      for (const movie of fetchedMoviesFromServer) {
        if (movie.comments && movie.comments.length > 0) {
          const dbComments: DbComment[] = movie.comments
            .sort(
              (a, b) =>
                new Date(b.createdAt).getTime() -
                new Date(a.createdAt).getTime()
            )
            .slice(0, 2); // Sort by date and take top 2

          const displayComments: DisplayComment[] = dbComments.map(
            (comment) => ({
              id: comment.id,
          ...
          newValidReviewsCollectedInThisBatch.push({
            ...
            movie: {
              ...
              rating:
                movie.rating?.RatingValue !== undefined
